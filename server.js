...
io.on('connection', (socket) => {
    ...
    socket.on('changeUsername', async (newUsername) => {
        ...
        try {
            const alreadyAvailable = await UserModel.findOne({ userName: newUsername });
            if (alreadyAvailable) {
                return socket.emit('changeUsernameAnswer', "Username already taken");
            }
            ...
            await groupModel.updateMany(
                ...
                { $set: { createdBy: newUsernameValue } }
            );

            const groupsWithUserAsMember = await groupModel.find({ members: oldUsername });
            const groupBulkOps = groupsWithUserAsMember.map(group => ({
                updateOne: {
                    filter: { _id: group._id },
                    update: {
                        $set: {
                            members: group.members.map(member =>
                                member === oldUsername ? newUsernameValue : member
                            )
                        }
                    }
                }
            ...
            if (groupBulkOps.length > 0) {
                await groupModel.bulkWrite(groupBulkOps);
            }

            await groupMessageModel.updateMany(
                { from: oldUsername },
                { $set: { from: newUsernameValue } }
            );
            const groupMessagesWithUserAsMember = await groupMessageModel.find({ members: oldUsername });
            const groupMessageBulkOps = groupMessagesWithUserAsMember.map(message => ({
                updateOne: {
            ...
            if (groupMessageBulkOps.length > 0) {
                await groupMessageModel.bulkWrite(groupMessageBulkOps);
            }
            ...
            const friendsListsWithUser = await friendsModel.find({ friendsName: oldUsername });
            const friendsBulkOps = friendsListsWithUser.map(friend => ({
                updateOne: {
                    filter: { _id: friend._id },
                    update: {
                        $set: {
                            friendsName: friend.friendsName.map(name =>
                                name === oldUsername ? newUsernameValue : name
                            )
                        }
                    }
                }
            }));
            if (friendsBulkOps.length > 0) {
                await friendsModel.bulkWrite(friendsBulkOps);
            }

            await friendsMessageModel.updateMany(
                { from: oldUsername },
                { $set: { from: newUsernameValue } }
            );
            await friendsMessageModel.updateMany(
                { to: oldUsername },
                { $set: { to: newUsernameValue } }
            );

            socket.userName = newUsernameValue;

            socket.emit('changeUsernameAnswer', "Username changed successfully");
        } catch (error) {
            console.error("Error changing username:", error);
            socket.emit('changeUsernameAnswer', "Error changing username");
        }
    });
